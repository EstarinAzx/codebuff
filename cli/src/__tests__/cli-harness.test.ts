...
describe('CLI harness routing', () => {
  test('BYOK modes retain local templates and Freebuff turns run base3', () => {
    expect(CLI_HARNESS).toBe('base3')
    expect(AGENT_MODE_TO_ID.DEFAULT).toBe(IS_FREEBUFF ? 'base3' : 'mod-default')
    ...
    expect(AGENT_MODE_TO_ID.LITE).toBe(
      IS_FREEBUFF ? 'base2-free' : 'mod-lite',
    )
    expect(
  ...
  test('MAX and PLAN never followed the harness switch', () => {
    ...
    expect(AGENT_MODE_TO_ID.MAX).toBe(IS_FREEBUFF ? 'base2-max' : 'mod-max')
    expect(AGENT_MODE_TO_ID.PLAN).toBe(IS_FREEBUFF ? 'base2-plan' : 'mod-plan')
  })
