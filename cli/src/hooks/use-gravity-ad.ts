...
const BYOK_AD_STATE: GravityAdState = {
  ...
  isLoading: false,
  recordImpression: () => {},
  // PORT: upstream's ad-banner refactor added recordClick to GravityAdState —
  // no-op it here so the BYOK early-return still satisfies the widened type.
  recordClick: () => {},
}
...
export const useGravityAd = (options?: GravityAdOptions): GravityAdState => {
  // PORT: skip the backend ad pipeline in standalone BYOK mode.
