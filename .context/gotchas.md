...
Fix pattern: paint an opaque `backgroundColor` on the PARENT box — children can stay transparent, they composite over the parent's fill. Since v1.1.2 the chat input box AND the askUser questions box paint the terminal's OWN background color, queried once per process via OSC 11 (`cli/src/hooks/use-terminal-background.ts` → `renderer.getPalette()`), `#000000` fallback when the terminal never answers — opaque but visually identical to transparent. Compact mode and the status bar use `theme.surface`. Reuse the hook for any future invisible-but-opaque fill. OpenTUI has NO "SGR 49 / terminal default bg" paint primitive (explicit RGBA or alpha-0 only) — that's why the query approach exists.
