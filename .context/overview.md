...
**Local upgrade installed:** `modded` includes subscription search plus local browser/Windows control without another service API key. Global `rdx` runs `1.5.2-dev.tools.1`; provider profiles are preserved. See [[active-work]] and [tool usage](../docs/computer-tools.md).
...
Upstream is `CodebuffAI/codebuff`. The fork was renamed `EstarinAzx/codebuff` â†’ `EstarinAzx/codebuff-modded` between 1.0.1 and 1.0.2 ships. Canonical release home is `EstarinAzx/codebuff-modded`. Divergence is deep â€” upstream merges are merge-and-resolve, not drop-in.

## Map

- [[flows]] â€” traced search and local-tool extension points

- [[pick-up]] â€” the next session's handoff
- [[happy-path]] â€” intended coding journey
- [[stack]] â€” tech, build commands, workspaces
- [[active-work]] â€” what's in flight on the `modded` branch
- [[decisions]] â€” rationale for fork-local edits
- [[gotchas]] â€” non-obvious behaviors when touching this repo
- [MERGE-STRATEGY.md](../MERGE-STRATEGY.md) â€” how to sync upstream â†’ main â†’ modded safely (read BEFORE any upstream merge)

## Authoritative project docs (do not duplicate here)

Maintained upstream â€” read these directly:

- `AGENTS.md` â€” entry point, conventions, doc index
- `docs/architecture.md` â€” package graph, per-package details
- `docs/request-flow.md` â€” full request lifecycle, CLI â†’ server â†’ back
- `docs/error-schema.md` â€” server error formats
- `docs/development.md` â€” dev setup, worktrees, logs, DB migrations
- `docs/testing.md` â€” DI-over-mocking, tmux CLI testing
- `docs/environment-variables.md` â€” env vars, DI helpers, loading order
- `docs/agents-and-tools.md` â€” agent system, shell shims, tool definitions
...
- **CLI/TUI** â€” `cli/` (OpenTUI + React)
- **SDK** â€” `sdk/` (JS/TS, consumed by CLI + external users)
- **Agent runtime** â€” `packages/agent-runtime/` (server-side tool dispatch)
- **Agent templates** â€” `agents/` (shipped) and `.agents/` (local)
